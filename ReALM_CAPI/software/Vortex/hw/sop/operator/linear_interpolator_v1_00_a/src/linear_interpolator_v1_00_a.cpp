...
double linear_interpolator_v1_00_a::ToDouble(int64_t X, int NumFractionBits)
{
	return (double)X / std::pow(2, NumFractionBits);
}

double linear_interpolator_v1_00_a::log2(double X)
{
	return log(X) / log(2.0);
}

double roundReALM(double number)
{
    return number < 0.0 ? ceil(number - 0.5) : floor(number + 0.5);
}

linear_interpolator_v1_00_a::linear_interpolator_v1_00_a(uint64_t BaseAddress, vector<CoefficientTableEntry*> CoefficientTable, vector<RangeTableEntry*> RangeTable)
{
...
linear_interpolator_v1_00_a::linear_interpolator_v1_00_a(uint64_t BaseAddress, InterpolatedFunction Function, double DomainMin, double DomainMax, int DomainNumIntegerBits, int DomainNumFractionBits, int RangeNumIntegerBits, int RangeNumFractionBits, double MaxError)
{
	m_ConfigBaseAddress[0] = 0x00000;
	m_ConfigBaseAddress[1] = 0x20000;
	...
	m_DomainFractionScale = (int64_t)roundReALM(pow(2,m_DomainNumFractionBits));
...
linear_interpolator_v1_00_a::~linear_interpolator_v1_00_a()
{
	if (m_IsLocalInit)
	{
		for(std::vector<CoefficientTableEntry*>::iterator it = m_CoefficientTable.begin(); it != m_CoefficientTable.end(); ++it)
			delete(*it);

		for(std::vector<RangeTableEntry*>::iterator it = m_RangeTable.begin(); it != m_RangeTable.end(); ++it)
			delete(*it);

		for(std::vector<InterpolationRangeGroup*>::iterator it = m_ConsecutiveRanges.begin(); it != m_ConsecutiveRanges.end(); ++it)
		{
			vector<InterpolationRange*> ranges = (*it)->GetConsecutiveRanges();
			for (std::vector<InterpolationRange*>::iterator rg = ranges.begin(); rg != ranges.end(); ++rg)
			{
				delete(*rg);
			}
...
int linear_interpolator_v1_00_a::GetInputWidth()
{
	return m_DomainNumIntegerBits + m_DomainNumFractionBits;
}

int linear_interpolator_v1_00_a::GetOutputWidth()
{
	return m_RangeNumIntegerBits + m_RangeNumFractionBits;
}

vector<CoefficientTableEntry*> linear_interpolator_v1_00_a::GetCoefficientTable()
{
	return m_CoefficientTable;
}

vector<RangeTableEntry*> linear_interpolator_v1_00_a::GetRangeTable()
{
	return m_RangeTable;
}

vector<vector<int64_t>> linear_interpolator_v1_00_a::Interpolate(vector<vector<int64_t>> Image)
{
	double MaxError = 0;
...
int64_t linear_interpolator_v1_00_a::InterpolateValue(int64_t Value)
{
	RangeTableEntry* SelectedRange;
	...
	CoefficientTableEntry* SelectedCoefficient = m_CoefficientTable.at(CoefficientTableAddress);

	return ComputeMxPlusB(Value - SelectedCoefficient->GetXOffset(), SelectedCoefficient->GetSlope(), SelectedCoefficient->GetYOffset());
}

vector<VortexConfigurationPacket*> linear_interpolator_v1_00_a::GetConfiguration()
{
...
VortexConfigurationPacket* linear_interpolator_v1_00_a::GetCoefficientTableConfiguration()
{
...
VortexConfigurationPacket* linear_interpolator_v1_00_a::GetRangeTableConfiguration()
{
...
void linear_interpolator_v1_00_a::WriteSimulationFiles(string Filepath, vector<vector<int64_t>> Input)
{
...
void linear_interpolator_v1_00_a::WriteSimulationConfigurationFiles(string Filename)
{
...
void linear_interpolator_v1_00_a::WriteToFile(string Filename, vector<vector<int64_t>> Data, int NumBits, int NumBitsPerLine)
{
	ofstream sw(Filename,std::ios_base::trunc);
...
bool linear_interpolator_v1_00_a::InitRange()
{
	...
	vector<InterpolationRange*> RangeInfo;
	RangeInfo.push_back(new InterpolationRange(m_IFunction,m_DomainStep,m_DomainMin));

	InterpolationRange* CurrentRange;
	double error;
	...
	while ((CurrentRange = RangeInfo.back())->GetRangeEnd() <= m_DomainMax)
	{
		if ((error = CalculateError(CurrentRange)) > m_MaxInterpolatorError)
		{
			...
			RangeInfo.push_back(new InterpolationRange(m_IFunction, m_DomainStep, CurrentRange->RemoveHalf()));
		}
		else
		{
...
void linear_interpolator_v1_00_a::InitRangeTable()
{
	m_RangeTable.clear();
	...
	for (int i=0; i<m_ConsecutiveRanges.size(); i++)
	{
		...
		m_RangeTable.push_back(new RangeTableEntry(RangePoints[0],RangePoints[1],RangeShift,RangeTableOffset));
		RangeTableOffset += m_ConsecutiveRanges[i]->GetNumRanges();
	}

	sort(m_RangeTable.begin(),m_RangeTable.end(),RangeTableEntry::Compare);
}

void linear_interpolator_v1_00_a::InitCoefficientTable()
{
	int CoefficientTableIndex = 0;
	int CoefficientTableSize = 0;
	m_CoefficientTable.clear();

	for(std::vector<InterpolationRangeGroup*>::iterator j=m_ConsecutiveRanges.begin();j!=m_ConsecutiveRanges.end();++j)
	...
	for (int g=0; g<m_ConsecutiveRanges.size(); g++)
	{
		...
		{
			InterpolationRange* ir = m_ConsecutiveRanges[g]->GetConsecutiveRanges()[r];
			...
			m_CoefficientTable.push_back(new CoefficientTableEntry(Coefficients[0],Coefficients[1],Coefficients[2],ir));
		}
	}
}

vector<InterpolationRangeGroup*> linear_interpolator_v1_00_a::CompressRanges(vector<InterpolationRange*> Ranges)
{
	int index = 0;
	vector<InterpolationRangeGroup*> ConsecutiveRanges;

	while (index < Ranges.size())
	{
		int collapsed_ranges_count = 0;
		InterpolationRangeGroup* irg = NULL;
		for(std::vector<InterpolationRange*>::iterator it = Ranges.begin() + index; it != Ranges.end(); ++it)
		{
			if ((*it)->GetNumIntervals() == Ranges[index]->GetNumIntervals())
			{
				collapsed_ranges_count++;
				if (irg == NULL)
					irg = new InterpolationRangeGroup();
...
int64_t linear_interpolator_v1_00_a::ComputeMxPlusB(int64_t x, int64_t slope, int64_t b)
{
	int64_t mx = slope * x;
...
double linear_interpolator_v1_00_a::CalculateError(InterpolationRange* ir)
{
	double MaxError = 0;

	if (ir->GetNumInterpolatedPoints() == 1)
		return abs(m_IFunction(ir->GetRangeStart()) - ((int64_t)roundReALM(m_IFunction(ir->GetRangeStart())* m_RangeFractionScale) / (double)m_RangeFractionScale));

	int64_t fxd_intercept = (int64_t)roundReALM(ir->GetIntercept() * m_TableFractionScale);
	int64_t startPoint = (int64_t)roundReALM(ir->m_Points[0] * m_DomainFractionScale);

	for (int i=0; i<ir->m_Points.size(); i++)
	{
		int64_t fxd_slope = (int64_t)roundReALM(ir->GetSlope() * m_TableFractionScale);
...
int64_t linear_interpolator_v1_00_a::ToInt64(double X, int NumFractionBits, ConvertMethod ConvMethod)
{
	if (ConvMethod == ConvertMethod::ROUND)
		return (int64_t)roundReALM(pow(2,NumFractionBits) * X);
