...
double multichannel_histogram_v1_00_a::log2(double X)
{
	return log(X) / log(2.0);
}

multichannel_histogram_v1_00_a::multichannel_histogram_v1_00_a(uint64_t BaseAddress, int NumHistograms, int NumHistogramBins)
{
...
multichannel_histogram_v1_00_a::~multichannel_histogram_v1_00_a(void)
{
}

int multichannel_histogram_v1_00_a::GetNumHistogramBins()
{
	return m_NumHistogramBins;
}

int multichannel_histogram_v1_00_a::GetHistogramIndexWidth()
{
	return ceil(log2(m_NumHistogramBins) + log2(m_NumHistograms));
}

void multichannel_histogram_v1_00_a::SetPartition(histogram_partition* Partitions)
{
	m_Partitions = Partitions;
}

void multichannel_histogram_v1_00_a::Init()
{
	m_Bins.clear();
...
vector<VortexConfigurationPacket*> multichannel_histogram_v1_00_a::GetConfiguration(vector<vector<int>> MinMaxConfig, int ImageWidth, int ImageHeight, histogram_partition* Partitions)
{
...
<<<<<<< HEAD
/*
	VortexConfigurationPacket* vcp[2];
	vcp[0] = new VortexConfigurationPacket(MinMaxConfigurationBytes.data(),MinMaxConfigurationBytes.size(), m_MinMaxConfigBaseAddress);
	vcp[1] = new VortexConfigurationPacket(ImageSizeConfigurationBytes.data(), ImageSizeConfigurationBytes.size(), m_ImageSizeConfigBaseAddress);
*/
=======
	/*
	VortexConfigurationPacket* vcp[2];
...
vector<uint8_t> multichannel_histogram_v1_00_a::GetImageSizeConfiguration(int ImageWidth, int ImageHeight)
{
...
vector<uint8_t> multichannel_histogram_v1_00_a::GetMinMaxConfiguration(vector<vector<int>> MinMaxConfig)
{
...
vector<uint8_t> multichannel_histogram_v1_00_a::GetPartitionConfiguration()
{
...
vector<int64_t> multichannel_histogram_v1_00_a::BuildHistogram(vector<vector<int64_t>> Image, int RangeMin, int RangeMax)
{
	Init();
...
vector<vector<int64_t>> multichannel_histogram_v1_00_a::IndexHistogram(vector<vector<int64_t>> Image, int RangeMin, int RangeMax)
{
	int Height = Image.size();
	int Width = Image[0].size();
...
vector<vector<int64_t>> multichannel_histogram_v1_00_a::GenerateHistogramTestData(vector<vector<int64_t>> Image, vector<int> MinMax, string OutputFilepath)
{
...
void multichannel_histogram_v1_00_a::WriteToFile(vector<vector<int64_t>> Image, int OutputDatawidth, ofstream* sw)
{
...
void multichannel_histogram_v1_00_a::WriteToFile(vector<int64_t> Data, int OutputDatawidth, ofstream* sw)
{
    uint64_t Mask = 0xFFFFFFFFFFFFFFFF >> (64 - OutputDatawidth);
    int formatWidth = (int)ceil((float)OutputDatawidth / 4);
