#include "VisionServerNetworkUtility.hpp"

VisionServerNetworkUtility::VisionServerNetworkUtility(){
...
#ifdef __WIN32
		VisionServerNetworkUtility::VisionServerNetworkUtility(SOCKET sock){
			connectedSocket = sock;
		}
#endif
#ifdef __linux__
		VisionServerNetworkUtility::VisionServerNetworkUtility(int socket){
			connected_socket = socket;
		}
#endif

SocketData * VisionServerNetworkUtility::receiveDataPacket(int * close){
	*close = 0;
...
void * VisionServerNetworkUtility::receiveDataPacketThread(void * data){
...
int VisionServerNetworkUtility::receiveDataPacketAsync(void (*receiveCallback)(SocketData *)){
#ifdef __linux__
...
void VisionServerNetworkUtility::sendDataPacket(SocketData * data){
...
void VisionServerNetworkUtility::sendDataPacketAsync(SocketData * data){
}
uint8_t * VisionServerNetworkUtility::receiveLength(int length){
    uint8_t * data = NULL;
    #ifdef _WIN32
...
void VisionServerNetworkUtility::sendLength(uint8_t * data, uint32_t length){
    #ifdef _WIN32
...
int VisionServerNetworkUtility::openSocket(const char * ipaddress, uint32_t portnumber){
    #ifdef __WIN32
...
void VisionServerNetworkUtility::closeSocket(){
    #ifdef __WIN32
...
void VisionServerNetworkUtility::send_tcp_packet(uint8_t * data, uint32_t length){
    #ifdef _WIN32
        send_tcp_packet_windows(data, length);
    #endif
    #ifdef __linux__
...
#ifdef _WIN32
int VisionServerNetworkUtility::open_windows_tcp_socket(const char * ipaddress, uint32_t portnumber){
...
void VisionServerNetworkUtility::close_windows_tcp_socket(){
    closesocket(connectedSocket);
}
void VisionServerNetworkUtility::send_tcp_packet_windows(uint8_t * data, uint32_t length){
...
uint8_t * VisionServerNetworkUtility::receive_length_windows(int length){
...
#ifdef __linux__
uint8_t * VisionServerNetworkUtility::receive_length_linux(int length){
    uint8_t * data = (uint8_t *)malloc(length);
	int remaining = length;
	int idx = 0;
	int bytes_received;
...
int VisionServerNetworkUtility::open_linux_tcp_socket(const char * ipaddress, uint32_t port_number){
    //Create socket
...
void VisionServerNetworkUtility::close_linux_tcp_socket(){
    close(connected_socket);
}
void VisionServerNetworkUtility::send_tcp_packet_linux(uint8_t * data, uint32_t length){
