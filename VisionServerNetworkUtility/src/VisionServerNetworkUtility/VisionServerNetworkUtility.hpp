#ifndef __VISION_SERVER_NETWORK_UTILITY_H__
#define __VISION_SERVER_NETWORK_UTILITY_H__
...
class VisionServerNetworkUtility;

struct receiveThreadArgs{
	VisionServerNetworkUtility * self;
	SocketData ** data;
...
class VisionServerNetworkUtility{
	public:
		VisionServerNetworkUtility();
		~VisionServerNetworkUtility();
#ifdef __WIN32
		VisionServerNetworkUtility(SOCKET sock);
#endif
#ifdef __linux__
		VisionServerNetworkUtility(int socket);
#endif
