 ...
 *      Author: root
 */
#include "AIMPostProcessWorkConsumer.h"

AIMPostProcessWorkConsumer::AIMPostProcessWorkConsumer() {
	// TODO Auto-generated constructor stub
}
AIMPostProcessWorkConsumer::~AIMPostProcessWorkConsumer() {
	// TODO Auto-generated destructor stub
}
void AIMPostProcessWorkConsumer::init(){
}
void AIMPostProcessWorkConsumer::runWork(WorkItem * w){
	...
	auto tb = std::chrono::system_clock::now();
									std::cout << "AIM POST PROCESS TIME: "
...
void AIMPostProcessWorkConsumer::deInit(){

}
void AIMPostProcessWorkConsumer::parseParams(uint8_t * params, uint32_t paramsLength){
	if(paramsLength >= 12){
...
void AIMPostProcessWorkConsumer::configure(){
...
uint8_t * AIMPostProcessWorkConsumer::postProcess(uint8_t * rawResult, uint32_t * length){
	...
	for (size_t k = 0; k < contours.size(); k++ )
	{
		...
		if ((area <= 10000.0f) && (area >= 50.0f))
		{
			...
			//if(boundingBox.width > 20 && boundingBox.height > 20 && boundingBox.width <= 128 && boundingBox.height <= 128){
			x = boundingBox.x;
...
cv::Mat AIMPostProcessWorkConsumer::GetSalienceMask(const cv::Mat &infomap, double thresholdPercent, int ErosionSize, int ClosureSize){
...
double AIMPostProcessWorkConsumer::calculatePercentile(const cv::Mat &infomap, double percentile){
#pragma omp for
...
double AIMPostProcessWorkConsumer::calcPercentile(const cv::Mat &sequence, double percentile)
{
