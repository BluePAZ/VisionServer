...
void CMT::initialize(const Mat im_gray, const Rect rect)
{
    ...
    int err = OpenCVSurfSupport::openCVSurfDetect(im_gray, keypoints);
    if(err){
        //revert to old call
    ...
    err = OpenCVSurfSupport::openCVSurfCompute(im_gray, keypoints_fg, descs_fg);
    if(err){
        //revert to old call
        descriptor->compute(im_gray, keypoints_fg, descs_fg);
    }
    err = OpenCVSurfSupport::openCVSurfCompute(im_gray, keypoints_bg, descs_bg);
    if(err){
        //revert to old call
        descriptor->compute(im_gray, keypoints_bg, descs_bg);
    }
...
void CMT::processFrame(Mat im_gray) {
    ...
    err = OpenCVSurfSupport::openCVSurfCompute(im_gray, keypoints, descriptors);
    if(err){
        //revert to old call
        detector->detect(im_gray, keypoints, descriptors);
    }
